/*
 * CuTest 实现（第三方，见 CuTest.h 头部许可说明）。
 * 本地修改：sprintf/vsprintf → snprintf/vsnprintf；循环变量统一为 int。
 */
#include <assert.h>
#include <setjmp.h>
#include <stdlib.h>
...
char *CuStrAlloc(int size) {
    return (char *) malloc(sizeof(char) * (size_t) size);
}

char *CuStrCopy(const char *old) {
    size_t len    = strlen(old);
    char  *newStr = CuStrAlloc((int) len + 1);
    memcpy(newStr, old, len + 1);
    return newStr;
}
